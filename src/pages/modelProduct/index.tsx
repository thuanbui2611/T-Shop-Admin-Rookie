...
import {
  deleteModelProductAsync,
  getModelProductsAsync,
  modelProductSelectors,
  setModelProductParams,
} from "./ModelProductSlice";
import ModelProductForm from "./ModelProductForm";
import Pagination from "../../app/components/Pagination";
import "./../../app/assets/css/custom.css";
import { useSearchParams } from "react-router-dom";

export default function ModelProductPage() {
  ...
  const [selectedModelProduct, setSelectedModelProduct] =
    useState<ModelProduct | null>(null);
  const [confirmDeleteDiaglog, setConfirmDeleteDiaglog] = useState(false);
  const [modelProductDeleted, setModelProductDeleted] = useState<ModelProduct>(
    {} as ModelProduct
  );

  const modelProducts = useAppSelector(modelProductSelectors.selectAll);
  const { modelProductLoaded, metaData, modelProductParams } = useAppSelector(
    (state) => state.modelProduct
  );
  ...
  useEffect(() => {
    if (!pageNum || pageNum === "1") {
      ...
      dispatch(setModelProductParams({ pageNumber: 1 }));
    } else {
      dispatch(setModelProductParams({ pageNumber: +pageNum }));
    }
  }, [pageNum, dispatch]);
  //Starting filter
  useEffect(() => {
  ...
  }, [modelProductParams, pageNum]);

  const handleSelectModelProduct = (
    actionName: string,
    modelProduct?: ModelProduct
  ) => {
    setOpenEditForm((cur) => !cur);
    if (modelProduct) {
      setSelectedModelProduct(modelProduct);
    }
  ...
  const cancelEditForm = () => {
    setOpenEditForm((cur) => !cur);
    setSelectedModelProduct(null);
  };

  async function handleDeleteModelProduct(modelProductDeleted: ModelProduct) {
    await dispatch(deleteModelProductAsync(modelProductDeleted.id));
  }
  const openConfirmDeleteDiaglog = (modelProduct: ModelProduct) => {
    setConfirmDeleteDiaglog((cur) => !cur);
    setModelProductDeleted(modelProduct);
  };
  const cancelConfirmDeleteDiaglog = () => setConfirmDeleteDiaglog(false);

  useEffect(() => {
    if (!modelProductLoaded) {
      if (isStartFilter || modelProducts.length === 0) {
        dispatch(getModelProductsAsync());
      }
    }
  }, [dispatch, modelProductParams, isStartFilter]);
  if (!metaData) {
    return <Loader />;
  } else
    return (
      <>
        ...
        <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
          <div className="flex justify-end">
            <button
              onClick={() => handleSelectModelProduct("Add Model")}
              type="button"
            ...
            >
              ...
              <span>Add new model product</span>
            </button>
          </div>
            ...
            <table className="w-full table-auto">
              ...
              <tbody>
                {modelProductLoaded ? (
                  ...
                  <>
                    {modelProducts.map((modelProduct) => (
                      <tr key={modelProduct.id}>
                        ...
                        <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                          <div className="flex items-center space-x-3.5">
                            ...
                            <button
                              ...
                              onClick={() =>
                                openConfirmDeleteDiaglog(modelProduct)
                              }
                            >
                              <svg
                                className="fill-current"
          ...
          <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-center sm:justify-center">
            <Pagination
              ...
              loading={modelProductLoaded}
            />
          </div>
        </div>
        {openEditForm && (
          <ModelProductForm
            modelProduct={selectedModelProduct}
        ...
        {confirmDeleteDiaglog && (
          <ConfirmDeleteDialog
            objectName={modelProductDeleted.name}
            actionDelete={() => handleDeleteModelProduct(modelProductDeleted)}
