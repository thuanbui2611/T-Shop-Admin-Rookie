...
export default function TypePage() {
  ...
  const [typeDeleted, setTypeDeleted] = useState<Type>({} as Type);

  const types = useAppSelector(typeSelectors.selectAll);
  const { typeLoaded, metaData, typeParams } = useAppSelector(
    (state) => state.type
  );
  ...
  useEffect(() => {
    if (!pageNum || pageNum === "1") {
      ...
      dispatch(setTypeParams({ pageNumber: 1 }));
    } else {
      dispatch(setTypeParams({ pageNumber: +pageNum }));
    }
  }, [pageNum, dispatch]);
  //Starting filter
  useEffect(() => {
  ...
  const handleSelectType = (actionName: string, type?: Type) => {
  ...
  async function handleDeleteType(typeDeleted: Type) {
    await dispatch(deleteTypeAsync(typeDeleted.id));
  }
  const openConfirmDeleteDiaglog = (type: Type) => {
  ...
  useEffect(() => {
    if (!typeLoaded) {
      if (isStartFilter || types.length === 0) {
        dispatch(getTypesAsync());
      }
    }
  }, [dispatch, typeParams, isStartFilter]);
  if (!metaData) {
    return <Loader />;
  } else
    return (
      <>
        <Breadcrumb pageName="Type Product" />
        <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
          <div className="flex justify-end">
            <button
              onClick={() => handleSelectType("Add Type")}
              type="button"
            ...
            >
              ...
              <span>Add new type product</span>
            </button>
          </div>
                        ...
                        <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                          <div className="flex items-center space-x-3.5">
                            <button
                              ...
                              onClick={() =>
                                handleSelectType("Edit Model", type)
                              }
                            >
                              <svg
                            ...
                            <button
                              ...
                              onClick={() => openConfirmDeleteDiaglog(type)}
                            >
                              <svg
                                className="fill-current"
                                width="18"
        ...
        {confirmDeleteDiaglog && (
          <ConfirmDeleteDialog
            objectName={typeDeleted.name}
            actionDelete={() => handleDeleteType(typeDeleted)}
